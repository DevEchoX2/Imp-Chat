        ...
        {tab === 'node' && (
          <div className="space-y-6 p-2 animate-in fade-in slide-in-from-left-2">
            <div className="bg-zinc-900/40 p-4 rounded-2xl border border-white/5">
              ...
              <p className="text-[7px] text-zinc-600 mt-2 font-medium">Share this with peers to bridge connections across the internet.</p>
            </div>

            <div className="bg-zinc-900/40 p-4 rounded-2xl border border-white/5">
              <h3 className="text-[8px] font-black text-zinc-500 uppercase tracking-widest mb-3">Connect to Peer</h3>
        ...
        {tab === 'chats' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between px-2">
              <span className="text-[10px] font-black text-zinc-700 uppercase tracking-widest">BRIDGE ACTIVE</span>
              <button 
            ...
            {isCreatingGroup && (
              <div className="p-3 bg-zinc-900/60 border border-emerald-500/20 rounded-2xl animate-in zoom-in-95 duration-200">
                ...
                <div className="max-h-32 overflow-y-auto mb-3 space-y-1 pr-1">
                  ...
                  {contacts.length === 0 && <div className="text-[8px] text-zinc-700 text-center py-2 uppercase font-black">No peers to add</div>}
                </div>
                <button 
                  onClick={handleFinalCreateGroup}
                  disabled={!newGroupName || selectedMembers.length === 0}
                  className="w-full py-2 bg-emerald-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest disabled:opacity-30 disabled:grayscale transition-all"
                >
                  CREATE CLUSTER
                </button>
              </div>
              ...
              {groups.map(group => (
                ...
                >
                  ...
                  <div className="flex-1 text-left min-w-0">
                    ...
                    <div className={`text-[8px] font-black uppercase tracking-widest ${activeId === group.id ? 'text-zinc-500' : 'text-zinc-600'}`}>{group.members.length} PEERS IN MESH</div>
                  </div>
                </button>
              ))}

              {contacts.length === 0 && groups.length === 0 && !isCreatingGroup && (
                 <div className="py-12 text-center text-[8px] text-zinc-800 font-black uppercase tracking-widest border border-dashed border-zinc-900 rounded-2xl mx-2">
                    No active bridges found
                 </div>
              )}
              {contacts.map(user => (
                <div
                ...
                >
                  <div className="relative group/avatar" onClick={(e) => { e.stopPropagation(); onViewProfile(user.id); }}>
                    ...
                    <div className="absolute inset-0 bg-black/20 opacity-0 group-hover/avatar:opacity-100 rounded-full flex items-center justify-center transition-opacity">
                      <UserIcon size={12} className="text-white" />
                    </div>
                  </div>
                  <div className="flex-1 text-left min-w-0">
                    <div className="font-bold text-xs truncate">{user.username}</div>
                    <div className={`text-[8px] font-black uppercase tracking-widest ${activeId === user.id ? 'text-zinc-500' : 'text-zinc-600'}`}>Direct Node Connected</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[7px] text-emerald-500 font-black uppercase tracking-widest flex items-center gap-1 mt-0.5">
              <ShieldCheck size={8} /> ENCRYPTED NODE
            </div>
          </div>
